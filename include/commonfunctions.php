...
function checkTableName($shortTName, $type=false)
{
	...
	if ("receive" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("despatch" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("movement" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("receive");
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("despatch");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("movement");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function SetAuthSessionData($pUsername, &$data, $password, &$pageObject = null, $fireEvents = true )
{
	...
	$_SESSION["_receive_OwnerID"] = $data["active"];
